...
- `commit-msg`、`post-commit`、`post-rewrite`、`pre-push` hook
- 链式 hook 安装与恢复
- checkpoint ref 持久化
- checkpoint 查询、列表、重建
- review / handoff 文档生成
- rewrite lineage 记录
- installable 包导出
- 业务分支 push 时自动同步 checkpoint ref
...
`story enable` 会安装这四个 hook wrapper：
...
- 如果本地还没有 checkpoint ref，直接跳过
- 如果正在同步 checkpoint ref，会通过 `STORY_SYNCING_CHECKPOINTS=1` 防止递归触发
- 默认模式下，同步失败只记录到 `.story/runtime/hook-errors.log`，不阻断业务 push
- `story enable --strict` 后，同步失败会阻断业务 push
